...
export const SECTION_REGISTRY: SectionConfig[] = [
  {
    ...
    render: (ctx) => (
      <HeroGuide
        ...
        boardCount={ctx.algorithmBoards.length}
        readyBoardCount={ctx.algorithmBoards.filter((board) => board.status === "usable" || board.status === "verified").length}
  ...
  },
  {
    ...
    inNav: true,
    withDivider: true,
    render: (ctx) => <AlgorithmsSection isLight={ctx.isLight} boards={ctx.algorithmBoards} directories={ctx.algorithmDirectories} />,
  },
  {
    id: "projects",
    label: "PROJECTS",
...
export const NAV_ITEMS = SECTION_REGISTRY.filter((item) => item.inNav).map(({ id, label }) => ({ id, label }));
