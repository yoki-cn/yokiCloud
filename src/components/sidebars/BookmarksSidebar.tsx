...
import { Bookmark, ChevronDown, ChevronLeft, LayoutGrid, Newspaper, Radio, ShieldCheck } from "lucide-react";
...
function faviconCandidates(url: string, icon?: string) {
  ...
  try {
    const u = new URL(url);
    if (!custom && u.hostname !== "qoj.ac") list.push(`/logo/bookmarks/${u.hostname}.ico`);
  } catch {
    // ignore
  }
  return list;
...
function SidebarBlock({ id, icon: Icon, label, sublabel, badge, open, onToggle, children }: SidebarBlockProps) {
  ...
  return (
    <CutCorner className="inner-elevated yc-elevation-compact rounded-lg border-0 bg-white/5 p-3">
      <button
        type="button"
...
export default function BookmarksSidebar({ open, onOpenChange }: BookmarksSidebarProps) {
  ...
  const [adminOpen, setAdminOpen] = useLocalStorageState<boolean>("ark.portal.sidebar.block.admin.open.v1", false);
                ...
                <div className="bm-scroll mt-1 flex-1 min-h-0 overflow-auto">
                  <div className="flex flex-col gap-3 pr-1 pb-2">
                      ...
                      {SIDEBAR_BULLETINS.length ? (
                        <ul className="flex flex-col gap-2">
                          {SIDEBAR_BULLETINS.map((item) => (
                            <li key={item.id}>
                              <CutCorner className="inner-elevated yc-elevation-hairline sidebar-item-card rounded-lg border-0 bg-white/5 p-2.5">
                    ...
                    <SidebarBlock
                      id="sidebar_recent"
                      icon={Newspaper}
                      label="MILESTONES / 关键更新"
                      sublabel="IMPORTANT UPDATES"
                      badge="LATEST"
                      open={recentOpen}
                              ...
                              <li key={b.id}>
                                <CutCorner className="inner-elevated yc-elevation-hairline sidebar-item-card rounded-lg border-0 bg-white/5 p-2.5">
                                  <a
                                    ...
                                    target="_blank"
                                    rel="noreferrer"
                                    onClick={(e) => handleExternalLinkClick(e, b.url)}
                                    className="block"
                    ...
                    <SidebarBlock
                      ...
                      icon={ShieldCheck}
                      label="管理台 / ADMIN"
                      sublabel="LOGIN REQUIRED / 需要登录"
                      badge="LOCKED"
                      open={adminOpen}
                      onToggle={() => setAdminOpen((s) => !s)}
                    >
                      <CutCorner className="inner-elevated yc-elevation-hairline sidebar-item-card rounded-lg border-0 bg-white/5 p-2.5">
                        <a href={sitePath("/admin/")} className="block" title="进入内容管理台">
                          <div className="flex items-start gap-2">
                            ...
                            <div className="min-w-0">
                              <div className="text-[10px] tracking-[0.18em] text-white/75 hover:text-white/95">
                                进入管理台
                              </div>
                              <div className="mt-1 text-xs leading-relaxed text-white/55 text-muted-block">
                                需要管理员账号和密码。普通浏览不需要进入这里。
                              </div>
                            </div>
                          </div>
                        </a>
