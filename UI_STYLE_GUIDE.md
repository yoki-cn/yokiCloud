...
- 面板：半透明黑底 + shadow hairline + 接触/环境阴影，强调「浮层/面板堆叠」的深度层级。
...
实现位置：
- 主题开关：`src/hooks/useTheme.ts`
- 主题样式：`src/styles/localStyles.tsx`（使用 `[data-theme="light"]` 覆盖）
...
实现位置：
- 侧栏布局变量：`src/styles/localStyles.tsx`（`--sidebar-w/--sidebar-gap`）
- 主内容偏移：`src/styles/localStyles.tsx`（`.content-rail`）
...
- 面板容器：`src/components/ui/Panel.tsx`
  - 内置 cut-corner、header（label/sublabel/right）
- 无界容器：`src/components/ui/UnboundedSurface.tsx`
  - 用于说明、统计、轻量索引和辅助导航，外轮廓不使用可见 border
  - 不使用大面积混合模式或背景模糊，避免主题切换时出现合成层残影
- CutCorner：`src/components/ui/CutCorner.tsx`
  - 可用于「内层卡片」「块级容器」来保持风格统一
- Pills / Chips：`src/components/ui/Pill.tsx` + `.proj-hud-chip`（在 `localStyles.tsx`）
- Buttons：`src/components/ui/Buttons.tsx`
  - `IconButton` / `LinkButton` / `ExtRouteTag`
- 分割线：`src/components/ui/DividerLine.tsx`
- 章节标题：`src/components/ui/SectionTitle.tsx`

### 侧栏模块（Side Panel）

- 侧栏组件：`src/components/sidebars/BookmarksSidebar.tsx`
  - 公告/维护提示（Bulletin）
  - 新收录/最近更新（Recent）
  - 书签网格（Bookmarks Grid）
- 侧栏数据：
  - 书签：`src/data/bookmarks.ts`
  - 公告：`src/data/sidebar.ts`
...
- 自定义图标：
  - 把图标放到 `d:\Web\logo`
  - 在 `src/data/bookmarks.ts` 对应项填 `icon`
  - dev/build 会读取 `public/logo`（并进入 `dist/logo`）
...
2. **新增模块时复用 Panel/CutCorner**  
   ...
   - Surface 外轮廓使用 `--yc-shadow-panel`、`--yc-shadow-floating` 或 `--yc-shadow-modal`，不叠加可见 border。
   - 说明、统计和轻量导航优先使用 `UnboundedSurface`，降低整页硬边框密度。
   - 输入、分隔线、选中、错误和 focus 状态继续使用 border / outline。
...
- 全局风格（深浅模式、背景、细节）：`src/styles/localStyles.tsx`
- 顶栏布局/按钮顺序：`src/components/sections/HeaderTopBar.tsx`
- 侧栏模块与折叠逻辑：`src/components/sidebars/BookmarksSidebar.tsx`
- 书签配置：`src/data/bookmarks.ts`
- 公告配置：`src/data/sidebar.ts`
